...
the moment you hang up. Everything stays on your Mac; the only thing that ever
leaves it is the audio sent for transcription, which Tamlil deletes from the
provider as soon as the transcript comes back.

<p align="center">
  <img src="docs/assets/transcript.png" width="380"
       alt="A finished Tamlil meeting: Hebrew and English in one speaker-labeled transcript, with the recording playable inline below">
</p>
...
the Xcode Command Line Tools (`xcode-select --install`); the installer handles
everything else.
...
[console.soniox.com](https://console.soniox.com)), optionally connects Google
Calendar for meeting titles, and launches. The first time a call records, macOS
asks you to approve **Microphone** and **System Audio Recording** — say yes.
Re-running the command updates an existing install.
...
1. **Join a call.** When a call app (Zoom, Meet, Slack, and the like) starts
...
- **What leaves your machine.** Only two things. The two audio tracks are
  uploaded to Soniox's async speech-to-text API (default endpoint
  `api.soniox.com`) for transcription. If you connect Google Calendar, meeting
  titles and attendee names are read from it. Nothing else is sent anywhere —
  the learned lexicon and the read-only connector stay entirely local.
...
- **No permission prompts / silent recordings** — check Tamlil is enabled under
  ...
  ever changes.
- **`swift build` fails immediately** — install the Command Line Tools with
  `xcode-select --install`. There is no Xcode here, so `swift test` does not
  work; use `make test-swift`.
