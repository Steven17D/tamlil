...
#
# SPDX header gate: every first-party Python, Swift, and shell source
# must carry an `SPDX-License-Identifier: Apache-2.0` line so a license scanner
# or SBOM bot can resolve per-file provenance. Run from `make check` and CI.
#
