...
if [ -d "$DIR/.git" ]; then
  ...
  mkdir -p "$(dirname "$DIR")"
  git clone https://github.com/Steven17D/tamlil.git "$DIR" 2>/dev/null \
    || { command -v gh >/dev/null 2>&1 && gh auth status >/dev/null 2>&1 \
         && gh repo clone Steven17D/tamlil "$DIR"; } \
    || die "Clone failed; if the repository is still private, run: gh auth login"
fi
cd "$DIR"
...
# Point the pipeline at this checkout (its default is a ~/Projects dev path).
...
step "Transcripts will label your side as: $(id -F | cut -d' ' -f1)"
