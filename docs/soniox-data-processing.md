...
_Due-diligence summary for the privacy section. Confirmed against Soniox's own
public terms and API docs on 2026-07-20; their Terms of Service and Privacy
Policy both show "Last updated: June 29, 2026". Short quotes below are attributed
to the page that owns them. This records the terms that actually govern our
usage — self-serve terms — and does not imply a negotiated agreement we have
not entered._

## Bottom line

We upload raw meeting audio (containing named third parties) to Soniox's async
speech-to-text API using a self-serve API key against the default endpoint
`api.soniox.com`. Under that self-serve arrangement:
...
- **Retention — worse than we assumed.** "Zero retention" is the default only
  for real-time streaming. The **async API we use is Soniox's storage service**:
  the uploaded audio and the resulting transcript are stored server-side with
  **no automatic deletion** — retention is the caller's responsibility.
- **Our deletion — now complete.** Our pipeline deletes the stored transcription
  after each run (which also removes the uploaded audio file), so neither the
  audio nor the transcript text persists on Soniox under normal operation. See
  "Client-side deletion" below.
- **Contract basis — self-serve only.** We are bound by click-through Terms of
  Service plus Privacy Policy. A DPA is available self-serve through the Soniox
  Console but is **not** auto-incorporated by the Terms; unless someone accepted
  it in the Console, no DPA is in force. A HIPAA BAA requires additional written
  terms.
...
Practically: unless someone has signed into the Soniox Console and accepted the
self-serve DPA for our account, our legal basis is the click-through Terms of
...
we use. From the security-and-privacy docs: "Soniox does not store your audio or
transcript data unless explicitly requested through a service that supports
storage, i.e. async API." The Terms phrase it the same way: "Soniox does not
store Customer Content processed by the Services unless storage is explicitly
requested or configured by you, required to provide a requested product feature,
or otherwise agreed in writing."

Because our pipeline uses the async API, both the uploaded audio and the
resulting transcript are stored server-side, and Soniox does **not** delete them
for us. The async limits-and-quotas docs are explicit: "You must manually delete
files after obtaining transcription results. Files are not deleted
...
Our pipeline (`src/tamlil/transcribe_soniox.py`) creates a transcription
...
exists, and that file is deleted instead.) `--keep-remote` skips this cleanup.
Deletion is best-effort: a failed delete is logged but does not fail the run.

The consequence for public wording: after each run neither the uploaded audio
nor the transcript text is retained by Soniox under normal operation.
...
Soniox Console and open the Security & Compliance." We could not verify the
identities of the subprocessors, nor whether the (Console-gated) DPA promises
advance notice of new subprocessors, from any public source.
...
Because our pipeline calls the default `api.soniox.com`, our audio and
transcripts are processed and stored in the **US** unless a regional project was
explicitly requested and configured. Residency also does not cover system data:
per the Privacy Policy it "may not apply to system data such as account
...
(`trust.soniox.com` does not resolve). We have verified the framework *claims*
on Soniox's public pages but have **not** reviewed the underlying certificates or
...
- Whether our account has in fact accepted the self-serve DPA in the Console
  (this is an account-state check, not a public fact).

## Recommended follow-ups

1. **Delete the stored transcription, not just the file — done.** The
   pipeline now calls `DELETE /v1/transcriptions/{id}` after fetching the
   transcript, which also removes the associated file, so neither the audio nor
   the transcript text persists on Soniox after a run.
2. **Accept the self-serve DPA in the Console** so a data-processing agreement is
   actually in force, given we send audio containing named third parties.
3. **Consider the EU region and/or a BAA** if the recorded parties or data
   warrant it, since regional access and PHI handling both require contacting
   Soniox.
