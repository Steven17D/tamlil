...
Tamlil turns a Zoom or Slack call into a timestamped, speaker-labeled
...
```mermaid
flowchart TB
    callapp["Call app<br/>Zoom · Slack"]
...
- **Audio cleanup** (`denoise.py` with the bundled rnnoise model, the `echo/`
  package) — mic denoising and direction-aware echo suppression.
